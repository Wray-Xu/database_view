          ...
          <div class="pd-main-left">
            ...
            <div id="yunText" class="yun-text">
              <div
                ...
                :key="index"
              >
                <span :title="item.entBriefName">{{ item.entBriefName }}</span>          
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    let len = 6;
    this.textStyle = setInterval(() => {
      this.number = parseInt(Math.random() * 20, 6);
      if (len == 0) {
        len = 6;
        return this.dataList.sort(() => Math.random() - 0.5);
      } else {
        len--;
      }  
    }, 1000); 
  ...
  methods: {
    sendGet() {
      //格式b
      axios
      .get("https://www.agribigdata.net/avbts/getEnterprise", {
    // .get("http://localhost:8080/CloudRanch/getEnterprise", {
          params: {
          },
        })
        .then((resp) => {
          console.log(resp.data.Enterprise);
          this.dataList=resp.data.Enterprise
        })
        .catch((err) => {
          //
