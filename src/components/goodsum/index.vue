      ...
      <div class="detail_txt" :class="`detail_txt${index + 1}`"> 
        ...
        <div class="txt_right" :class="`txt_right${index + 1}`"> 
          <span> 
            ...
            <div class="s_div" :class="`s_div${index + 1}`">{{ item.number }}吨</div> 
          </span> 
        </div>
      </div>
...
.trigle {
  ...
  .first {//最下面第一层
    ...
    transform: rotateX(70deg) rotateZ(45deg);//缩放移动旋转
    perspective: 140px;
  ...
  .detail {//整体设置
    ...
    .detail_txt {
      ...
      width: calc(100% - 80px);//宽度比100%少80px
      height: 100%;
      .txt_right {
        padding: 10px 0;
        display: inline-block;
        float: right;//文字在百分比右边
        height: 60px;
        width: calc(100% - 65px);
        %arrow_icon {
          content: '';
